...
describe("App", () => {
  ...
  it("disables other install buttons while one install is already running", async () => {
    let resolveInstall!: (value: InstallApkResult) => void;
    const installPromise = new Promise<InstallApkResult>((resolve) => {
    ...
    for (const button of screen.getAllByRole("button", { name: "Install on Board" })) {
      expect(button).toBeDisabled();
    }

    resolveInstall(installSuccessResultFixture);

    expect(await screen.findByText("BE Home installed LuckyDice.apk on Board.")).toBeInTheDocument();
  });

  it("preserves host failure details for install errors", async () => {
    invokeMock.mockImplementation(async (command) => {
      if (command === "load_setup_gate_state") {
        return runnableFixture;
      }
      ...
      if (command === "install_apk_to_connected_board") {
        throw new Error("BE Home could not find the APK file at `C:\\Users\\Matt\\Downloads\\LuckyDice.apk` anymore.");
      }
    ...
    expect(
      await screen.findByText("BE Home couldn't finish that install request just yet."),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
